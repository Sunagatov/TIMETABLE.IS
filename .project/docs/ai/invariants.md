...
- frontend list query keys include normalized params so filter/sort/search changes refetch backend data
- frontend must keep separate filter state for Needs Review, Failures, and Approved

## Frontend invariants

- authenticated landing is Needs Review
- Approved view remains approved-only by default
- frontend is a backend client and must not invent local lifecycle transitions
- every user-visible action must be backed by a backend API call
- failed API calls must surface visible, useful errors
- 401 from any backend call returns the UI to login
